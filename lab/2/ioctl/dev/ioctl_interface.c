...
static int
ioctl_d_interface_setup_cdev(ioctl_d_interface_dev *ioctl_d_interface);
static int ioctl_d_interface_init(void);
static void ioctl_d_interface_exit(void);

static int
ioctl_d_interface_dev_init(ioctl_d_interface_dev *ioctl_d_interface) {
  int result = 0;
...
static void
ioctl_d_interface_dev_del(ioctl_d_interface_dev *ioctl_d_interface) {}
...
int ioctl_d_interface_open(struct inode *inode, struct file *filp) {
  ...
  if (!atomic_dec_and_test(&ioctl_d_interface->available)) {
    ...
    printk(KERN_ALERT "open ioctl_d_interface : the device has been opened by "
                      "some other device, unable to open lock\n");
...
long ioctl_d_interface_ioctl(struct file *filp, unsigned int cmd,
                             unsigned long arg) {
  switch (cmd) {
